 ...
 */
val columnChar = listOf<Char>('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h')
...
data class Square(val column: Int, val row: Int) {
    ...
    fun notation(): String = if (inside()) (columnChar[column - 1] + row.toString()) else ""
}
...
fun square(notation: String): Square =
    if (notation.matches(Regex("""[a-h][1-8]"""))) Square(
        columnChar.indexOf(notation.first()) + 1,
        notation.last().toString().toInt()
