...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    val substrCount = substrings.associateWith { 0 }.toMutableMap()
    File(inputName).bufferedReader().use { reader ->
        reader.forEachLine { line ->
            for ((str, _) in substrCount) {
                substrCount[str] =
                    substrCount[str]!! + "(?=\\$str)".toRegex(RegexOption.IGNORE_CASE).findAll(line).count()
            }
        }
        return (substrCount)
    }
...
fun centerFile(inputName: String, outputName: String) {
    val maxLen = File(inputName).readLines().maxOfOrNull { it.trim().length }
    File(outputName).bufferedWriter().use { writer ->
        File(inputName).forEachLine { line ->
            if (maxLen != null) {
                val length = (maxLen - line.trim().length) / 2
                if (maxLen != line.trim().length) {
                    writer.write(line.trim().padStart(length + (line.trim().length)))
                    writer.newLine()
                } else {
                    writer.write(line)
                    writer.newLine()
